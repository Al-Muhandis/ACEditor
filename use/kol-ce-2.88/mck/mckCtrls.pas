...
uses Windows, Messages, KOL, Classes, Forms, Controls, Dialogs, extctrls,
     stdctrls, comctrls, CommCtrl, SysUtils, Graphics, mirror,{$ifndef wince} ShellAPI,{$endif}
     mckObjs,
{$ifndef FPC}
//////////////////////////////////////////////////
     {$IFDEF _D6orHigher}                       //
...
//////////////////////////////////////////////////////////
     {$ENDIF}                                           //
{$else}
     ComponentEditors, PropEdits, LCLType,
{$endif FPC}
...
{$ENDIF}
  ...
  published
    ...
    property WordWrap;
    property IgnoreDefault default True;
...
{$ENDIF}
    function ParentMargin: Integer; override;
  public
    constructor Create( AOwner: TComponent ); override;
    destructor Destroy; override;
    function Pcode_Generate: Boolean; override;
  published
    ...
    property Brush;
    property ShowAccelChar: Boolean read FShowAccelChar write SetShowAccelChar default False;
    property AcceptChildren default True;
  end;
...
{$ENDIF}
  ...
  public
    ...
    function Pcode_Generate: Boolean; override;
  published
    property Align default caClient;
...
{$ENDIF}
  ...
  published
    ...
    property OnSplit;
    property Brush;
    property Align default caLeft;
    property Width default 4;
  end;
...
{$ENDIF}
    ...
    procedure SetupFirst( SL: TStringList; const AName, AParent, Prefix: String ); override;
    procedure P_SetupFirst( SL: TStringList; const AName, AParent, Prefix: String ); override;
    procedure DoAutoSize; {$ifdef FPC} reintroduce; {$endif}
  ...
  private
    ...
    procedure SetColor(const Value: TColor); {$ifdef FPC} reintroduce; {$endif}
  protected
    procedure SetupFirst( SL: TStringList; const AName, AParent, Prefix: String ); override;
    procedure P_SetupFirst( SL: TStringList; const AName, AParent, Prefix: String ); override;
{$IFDEF _D2009orHigher}
...
{$ENDIF}
  ...
  published
    ...
    property generateConstants: Boolean read FGenerateColIdxConst write SetGenerateColIdxConst default True;
    property Brush;
    property Unicode;
    {$IFNDEF _D2}
...
{$ENDIF}
  ...
  published
    property Transparent;
    property Options: TKOLTreeViewOptions read FOptions write SetOptions default [];
    property ImageListNormal: TKOLImageList read FImageListNormal write SetImageListNormal;
    property ImageListState: TKOLImageList read FImageListState write SetImageListState;
    property CurIndex: Integer read FCurIndex write SetCurIndex default 0;
    property TVRightClickSelect: Boolean read FTVRightClickSelect write SetTVRightClickSelect default False;
    property OnChange;
    property OnSelChange;
    property OnKeyDown;
    property OnKeyUp;
...
{$ENDIF}
    ...
    function IsBorderStored: boolean; override;
    procedure SetMargin(const Value: Integer); override;
  public
    function Pcode_Generate: Boolean; override;
    constructor Create( AOwner: TComponent ); override;
  ...
  published
    property TabStop default True;
    property Transparent;
    property Options: TTabControlOptions read FOptions write SetOptions default [];
    property ImageList: TKOLImageList read FImageList write SetImageList;
...
function TKOLButton.IsImageStored: boolean;
begin
  Result:=(Fimage <> nil) and (Fimage.Width <> 0) and (Fimage.Height <> 0);
end;
...
begin
  ...
  DrawText(Canvas.Handle,PChar(s),Length(s),R,Flag);
...
function TKOLPanel.ParentMargin: Integer;
begin
  if Parent is TKOLTabControl then
    Result:=0
  else
    Result:=inherited ParentMargin;
end;
...
procedure TKOLTabControl.CreateParams(var Params: TCreateParams);
begin
  ControlStyle := ControlStyle + [ csAcceptsControls ];
  inherited;
  ControlStyle := ControlStyle - [ csAcceptsControls ];
...
function TKOLTabControl.IsBorderStored: boolean;
begin
  Result:=Border <> 0;
end;

procedure TKOLTabControl.SetMargin(const Value: Integer);
begin
  inherited;
  FParentMargin:=False;
end;

function TKOLTabControl.BorderNeeded: Boolean;
begin
  Result:=True;
end;

procedure TKOLTabControl.Change;
begin
  AdjustPages;
  inherited;
end;
...
begin
  ...
  begin
    if S <> '' then
      S := S + ', ';
    C := PCharStringConstant(Self, 'Page' + IntToStr( I ) + 'Caption', Pages[ I ].Caption);
    {$IFDEF _D2009orHigher}
    if C <> '''''' then
     begin
      C2 := '';
...
{$ifdef FPC}
          TmpBmp.Canvas.Brush.Color:=$E1E2E3;
...
{$endif FPC}
  ...
  END;
  TRY
    SetBitmapFormat(Bmp, W, bitmap.Height, Format);
    for I := 0 to Items.Count - 1 do
    begin
...
{$ifdef FPC}
        Bt.Fpicture.OnChange := Bt.PictureChanged;
{$endif FPC}
      end
        else
      begin
    ...
    TRY
      ...
      begin
        ...
        begin
          ...
          begin
{$ifdef FPC}
            Bmp.Width := Bt.picture.Width;
            Bmp.Height := Bt.picture.Height;
            Bmp.Canvas.Brush.Color:=$E1E2E3;
...
{$endif FPC}
      ...
      SetBitmapFormat(Bmp, N * W, H, pf24bit);
      if FBmpTranColor <> clNone then
      begin
  ...
  begin
    SL.Add( Prefix + '{$ifndef wince} ' + AName + '.OnTBCustomDraw := nil; {$endif}' );
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
       ...
       {$ENDIF}
        if (B <> '') and (B[ 1 ] = '''') then
          Result := Result + 'PKOLChar(KOLString(''' + S + Copy( B, 2, MaxInt ) + '))'
        else
        if S <> '' then
          Result := Result + 'PKOLChar( KOLString( ''' + S + ''' + ' + B + ') )'
        else
          Result := Result + 'PKOLChar( KOLString( ' + B + ' ) )';
      end;
    end;
    if I < Items.Count-1 then
      Result := Result  + ', ';
...
begin
  ...
  begin
    ...
    begin
      I := Application.MessageBox('Some buttons have pictures assigned.'#13#10 +
...
begin
  ...
  if (ImageListNormal<>nil) and (ImageListNormal.Bitmap<>nil) then
  begin
    TMP:=TBitMap.Create;
...
{$ifdef FPC}
      GlobalDesignHook.RenameMethod(fOnClickMethodName, NewMethodName);
...
begin
  ...
  if IsHandlersEqual(TMethod(fOnClick), TMethod(Value)) then Exit;
  FonClick := Value;
  if not IsHandlerEmpty(TMethod( Value )) then
  begin
    if FToolbar <> nil then
    begin
      F := FToolbar.ParentForm;
      fOnClickMethodName := GetEventHandlerName(F, Self, 'onClick');
    end;
  end
    else
    FOnClickMethodName := '';
...
{$ifdef FPC}
procedure TKOLToolbarButton.PictureChanged(Sender: TObject);
begin
  Setpicture(Fpicture);
end;
{$endif FPC}
...
{$endif FPC}
  ...
  begin
    ...
    begin
      I := Application.MessageBox('Image list(s) will be detached from the toolbar.'#13#10 +
...
begin
  ...
  begin
    ...
    begin
      I := Application.MessageBox('Image list(s) will be detached from ' +
