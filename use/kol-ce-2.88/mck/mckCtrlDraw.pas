...
uses
  windows, graphics, mirror, mckctrls, extctrls, classes {$ifdef FPC}, GraphType{$endif};
...
begin
  ...
  begin
    if (curindex>=0) and (curindex<items.count) then s:=items[curindex] else s:='';
    s:=UTF8Decode(s);
    if hasborder then
    begin
{$ifdef FPC}
...
{$endif FPC}
    ...
    if s<>'' then drawtext(handle,pchar(s),length(s),r,DT_VCENTER or DT_SINGLELINE);
  end;
end;
