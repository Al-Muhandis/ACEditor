...
uses Windows, Messages, KOL, Classes, Forms, Controls, Dialogs, extctrls,
     stdctrls, comctrls, SysUtils, Graphics, mirror,
...
{$else}
     ComponentEditors, PropEdits, intfgraphics, GraphType,
{$endif FPC}
     TypInfo, menus;
...
{$ifdef FPC}
  ...
  if PixelFormat = pf24bit then
    ADesc.BitsPerPixel:=24
  else
    ADesc.BitsPerPixel:=32;
  img.DataDescription:=ADesc;
...
begin
  ...
  tm:=HD1.HeaderSize;
  HD1.HeaderSize:=(HD1.HeaderSize + 3) and not longint(3);
  HD1.NFFFF := $FFFF;
...
begin
  ...
  {$ifdef FPC}
  ExecuteWait('',
               'windres.exe -i "' + ProjectSourcePath + FileName + '.rc" -o "'
               + ProjectSourcePath + FileName + '.res"',
...
{$ifdef FPC}
    ...
    IL.Masked:=KIL.Masked;
    tbmp:=TBitmap.Create;
    try
      tbmp.Width:=IL.Width;
      tbmp.Height:=IL.Height;
      for i:=0 to KIL.Count - 1 do begin
        tbmp.Canvas.CopyRect(Rect(0, 0, tbmp.Width, tbmp.Height), KIL.bitmap.Canvas, Rect(i*IL.Width, 0, (i + 1)*IL.Width, IL.Height));
//        if IL.Masked then
//          IL.AddMasked(tbmp, Color2RGB( KIL.TransparentColor ))
//        else
          IL.Add(tbmp, nil);
      end;
    finally
...
{$endif FPC}
    ...
    ILCE := {$ifdef FPC} TComponentEditor {$endif}(GetComponentEditor( IL, Designer ));
    if ILCE <> nil then
    try
...
begin
  ...
  begin
    ...
    if not IsHandlerEmpty(TMethod(OnExecute)) then
      SL.Add( Prefix + AName + '.OnExecute := Result.' +
              GetEventHandlerName(ParentForm, Self, 'OnExecute') + ';' );
    if not IsHandlerEmpty(TMethod(OnSuspend)) then
      SL.Add( Prefix + AName + '.OnSuspend := Result.' +
              GetEventHandlerName(ParentForm, Self, 'OnSuspend') + ';' );
    if not IsHandlerEmpty(TMethod(OnResume)) then
      SL.Add( Prefix + AName + '.OnResume := Result.' +
              GetEventHandlerName(ParentForm, Self, 'OnResume') + ';' );
    if not IsHandlerEmpty(TMethod(OnDestroy)) then
      SL.Add( Prefix + AName + '.OnDestroy := Result.' +
              GetEventHandlerName(ParentForm, Self, 'OnDestroy') + ';' );
    if PriorityClass <> pcNormal then
      SL.Add( Prefix + AName + '.PriorityClass := ' +
              PriorityClasses[ PriorityClass ] + ';' );
    if ThreadPriority <> tpNormal then
  ...
  begin
    S := 'nil';
    if not IsHandlerEmpty(TMethod(OnExecute)) then
      S := 'Result.' + GetEventHandlerName(ParentForm, Self, 'OnExecute');
    SL.Add( Prefix + AName + ' := NewThreadEx( ' + S + ' );' );
  end;
  if not PriorityBoost then
    SL.Add( Prefix + AName + '.PriorityBoost := FALSE;' );
