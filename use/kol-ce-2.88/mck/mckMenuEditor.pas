...
{$ifdef FPC}
    GlobalDesignHook.SelectOnlyThis(MI);
    GlobalDesignHook.LookupRoot:=GetLookupRootForComponent(MenuComponent);
{$else Delphi}
...
{$ifndef FPC}
  {$IFNDEF _D5orHigher} // Bug in earlier Delphi2..Delphi4
  tvMenu.Items.EndUpdate;
  {$ENDIF}
{$endif}
...
{$endif FPC}
  ...
  begin
    ...
    begin
{$ifdef FPC}
      GlobalDesignHook.SelectOnlyThis(MenuComponent);
{$else Delphi}
      F := (MenuComponent as TKOLMenu).ParentForm;
      if F <> nil then
      begin
