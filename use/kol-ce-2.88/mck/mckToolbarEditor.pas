...
begin
  ...
  lvButtons.Parent := Self;
    ...
    lvButtons.Columns[ 0 ].Width := TWidth(-2);
...
var I: Integer;
    ...
    Toolbar: TKOLToolbar;
begin
  lvButtons{$ifndef FPC}.Items{$endif}.BeginUpdate;
  TRY
  ...
  FINALLY
    lvButtons{$ifndef FPC}.Items{$endif}.EndUpdate;
  END;
end;
...
begin
  ...
  VK_DELETE: {$ifndef FPC}if not lvButtons.IsEditing then {$endif} btnDelete.Click;
{$ifndef FPC}
...
{$ifdef FPC}
    ...
    GlobalDesignHook.LookupRoot:=GetLookupRootForComponent(ToolbarControl);
{$else Delphi}
...
begin
  if ToolbarControl <> nil then
    GlobalDesignHook.SelectOnlyThis(ToolbarControl);
end;
{$else Delphi}
procedure TfmToolbarEditor.SelectTB;
var F: TForm;
