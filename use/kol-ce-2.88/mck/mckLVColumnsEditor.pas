...
{$ifdef FPC}
    GlobalDesignHook.SelectOnlyThis(Col);
{$else Delphi}
...
var I: Integer;
    ...
    LV: TKOLListView;
begin
  lvColumns{$ifndef FPC}.Items{$endif}.BeginUpdate;
  TRY
  ...
  FINALLY
    lvColumns{$ifndef FPC}.Items{$endif}.EndUpdate;
  END;
end;
...
begin
  ...
  begin
    if LI.Index < lvColumns.Items.Count - 1 then
      LV.Cols.Move( lvColumns.Items.Count - 1, LI.Index + 1 );
...
begin
  ...
  VK_DELETE: {$ifndef FPC}if not lvColumns.IsEditing then {$endif}btDel.Click;
{$ifndef FPC}
...
begin
  ...
  LV.ActiveDesign := nil;
{$ifdef FPC}
  GlobalDesignHook.SelectOnlyThis(LV.ParentForm);
{$endif}
end;
...
begin
  GlobalDesignHook.SelectOnlyThis(FListView);
end;
{$else Delphi}
procedure TfmLVColumnsEditor.SelectLV;
var F: TForm;
