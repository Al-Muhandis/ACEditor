...
begin
  ...
  Move( PByte( cardinal( PBitsList( FromBits.fList ).fItems ) + cardinal((FromIdx + 31) div 32) )^,
        PByte( cardinal( PBitsList( fList ).fItems ) + cardinal(ToIdx div 32) )^, (N + 31) div 32 );
...
begin
  ...
  FillChar( PByte( cardinal( PBitsList( fList ).fItems ) + cardinal(FromIdx div 32) )^,
...
begin
  ...
  if OldCap < fList.Capacity then
    FillChar( PChar( cardinal( PBitsList( fList ).fItems ) + cardinal(OldCap * Sizeof( Pointer )) )^,
...
begin
  with Sender^, PMenu(Ctrl).Items[ItemID]{-}^{+} do begin
    if [upCaption,upAccelerator]*FUpdateMask <> [] then begin
      s:=Self.FCaption;
{$ifndef wince}
...
{$endif wince}
    ...
    if (upAccelerator in FUpdateMask) and (Self.FAccelerator.Key <> 0) then {YS}  // ƒобавить
